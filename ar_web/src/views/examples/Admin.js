            ...
            <div>
              <TabContent activeTab={"plainTabs" + plainTabs} className="mt-1">
                <TabPane tabId="plainTabs0">
                  ...
                  <div>
                    {data != null
                      ? data.pendingMuseums.map(
                          (pendingMuseum, index) => {
                            return (
                              <div>
                                <MuseumAdminCard
                                  pendingMuseum={pendingMuseum}
                                  setData={setData}
                                />
                              </div>
                            );
                ...
                <TabPane tabId="plainTabs1">
                  ...
                  <div>
                    {data != null
                      ? data.publicizedMuseums.map(
                          (publishedMuseum, index) => {
...
const MuseumAdminCard = (props) => {
  ...
  var museum = props.pendingMuseum.museum;

  var onAccept = () => {
    setLoading(true)
    admin.acceptPendingMuseum(props.pendingMuseum.userId).then((res) => {
  ...
  var onReject = () => {
    setLoading(true)
    admin.rejectPendingMuseum(props.pendingMuseum.userId).then((res) => {
      console.log(res);
      props.setData(admin.data)
      setLoading(false)
    });
